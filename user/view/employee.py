...
class Add_emp(APIView):
    def post(self , request , servicepk , categorypk = None ):
        ...
        if len(request.FILES) != 0:
            ...
            return Response({"error":"Please add image"}, status.HTTP_400_BAD_REQUEST)
            

        # checking data
        if len(cost) == 0 or len(description) == 0 or len(address) == 0:
            
            return Response({"error":"please fill your fields"}, status.HTTP_400_BAD_REQUEST)
        elif len(description) < 10:
            return Response({"error":"please fill proper description"}, status.HTTP_400_BAD_REQUEST)
        elif len(address) < 5:
            return Response({"error":"please fill proper address"}, status.HTTP_400_BAD_REQUEST)
        ...
        empserializer = EmployeeSerializer(data = data)
